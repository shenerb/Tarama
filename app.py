...
st.set_page_config(page_title="Şirket Kimlik OCR", page_icon="🪪", layout="centered")

st.title("🪪 Şirket Kimlik Kartı OCR (Editable + Dosya Hatası Düzeltilmiş)")
st.markdown("""
Bu uygulama EasyOCR kullanarak şirket kimlik kartlarından Ad/Soyad okur.  
OCR hatalarını manuel olarak tablo üzerinden düzeltebilir ve Excel olarak kaydedebilirsiniz.
""")
...
img_input = st.camera_input("Kameradan kimlik çek") if secim=="Kamera" else st.file_uploader("Kimlik dosyası yükle", type=["jpg","jpeg","png"])
...
if not st.session_state.veriler.empty:
    ...
    # Excel butonu -> manuel değişiklikler kaydedilir
    if st.button("⬇️ Excel olarak indir"):
        output = BytesIO()
    ...
    st.info("Henüz kimlik eklenmedi. Kamera veya dosya yükleyin.")
